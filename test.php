...
$t1 = microtime(true);
//$ck = new Client('tcp://172.16.0.116:9000', 'default', '123456', 'demo');
...
//    '`f7` Int64,', // 5.5 不支持
    ...
    '`f11` Decimal32(3),',
//    '`f12` Decimal64(5),', // 5.5 - 7.1 不支持
//    '`f13` Decimal128(7),', // 5.5 - 7.1 不支持
...
$data['select t6 int64'] = $ck->query("select id,f5 from t6 where f5=3");
...
$table                = [
    ...
    '`f1` UInt16,',
    '`f2` Datetime',
    ') ENGINE = MergeTree() ORDER BY (id, f1) SETTINGS index_granularity = 8192',
];
$data['create table'] = $ck->query(implode("\n", $table));
