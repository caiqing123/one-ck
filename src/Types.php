...
class Types
{
    ...
    protected function setNull(&$data)
    {
        if ($this->is_null) {
            ...
            array_unshift($this->is_null_data, 'C*');
            $this->write->addBuf(call_user_func_array("pack", $this->is_null_data));
        }
        $this->is_null_data = [];
    }
    ...
    protected function encode($data, $type, $real_type)
    {
        if (isset($this->base_types[$real_type])) {
            if ($real_type == 'uint64' || $real_type == 'int64') {
                $a = '';
                foreach ($data as $v) {
                    if ($real_type == 'uint64') {
                        $a .= pack('L' . '*', $v) . pack('L' . '*', 0);
                ...
                array_unshift($data, "{$this->base_types[$real_type][0]}*");
                $this->write->addBuf(call_user_func_array("pack", $data));
            }
